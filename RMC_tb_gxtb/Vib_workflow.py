...
def calc_vibration(row_describ, atoms):
    ...
    parprint(f'outstd of vib calculation for db entry {row_dc.db_id} with structure: {row_dc.structure_str}, adsorbate: {row_dc.adsorbate_str}')

    functional_folder = os.path.dirname(row_dc.db_path) + '/' + 'xtb_calc' # + sanitize(row_dc.xc) + ('_D4' if row_dc.dftd4 else '')
    ...
    calculation_setter(atoms=atoms, calc_params=calc_params)

    metal_symbol = ["Mg", "Sc", "Ti", "V", "Cr", "Mn", "Fe", "Co" , "Ni", "Cu", "Zn", "Y", "Zr", "Nb", "Mo", "Hf", "Ta", "W", "Ga", "In", "Sn", "Pb", "Bi"]
